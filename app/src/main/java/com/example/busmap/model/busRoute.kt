...
data class BusRoute(
    ...
    // Thêm field mới để lưu detailed route
    ...
    fun getDisplayPoints(): List<GeoPoint> {
        return if (detailedPoints.isNotEmpty()) {
            detailedPoints // Dùng detailed route nếu có
        } else {
            points // Fallback về station points
        }
    }

    // Function để lấy station points cho markers
    fun getStationPoints(): List<GeoPoint> {
        return points
    }
}